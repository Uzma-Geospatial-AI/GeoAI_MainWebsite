...
for u in d['units']:
    ...
    body+=f'<p class="source-download"><a href="../{source}" download>Download the complete original {"brochure (PDF)" if u["document"]=="brochure" else "presentation (PowerPoint)"} ↗</a></p>'
    ...
    pages.append(dict(slug=u['id'],title=u['title'],category=category,html=body,words=len(BeautifulSoup(body,'html.parser').get_text().split()),media=['../'+u['preview']],documentUnit=u['id'],url='../'+source))
...
resource='<section class="document-library" id="document-library" data-document-collection=""><div class="wrap"><p class="eyebrow">The complete collection</p><h2>More to discover.<br/>Nothing left behind.</h2><p class="collection-intro">All 12 brochure chapters and 8 presentation slides, with their photography, diagrams and source text.</p><div class="resource-downloads"><a class="button button-orange" href="resources/geoai-brochure-2026.pdf" download>2026 brochure · PDF ↗</a><a class="button" href="resources/geoai-projects-news.pptx" download>Projects &amp; news · PowerPoint ↗</a></div><details><summary>Browse all 20 chapters and slides</summary><div class="resource-grid">'+''.join(card(u['id']) for u in d['units'])+'</div></details></div></section>'
